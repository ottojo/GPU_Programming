...
def create_plane():
    ...
    for i in range(250):
        for j in range(250):
            ...
            rendVert.append(1.0)
            rendVert.append((1.0/249.0)*float(j))
            rendVert.append((1.0/249.0)*float(i))
...
class Exc5(WindowInterface):
    def __init__(self):
        self.ctx = moderngl.create_context()
        vs_path = Path.cwd() / 'exc5.vert'
        fs_path = Path.cwd() / 'exc5.frag'
        ...
        self.dispTexLoc = self.prog['displTexture']
        ...
        self.vao = self.ctx.vertex_array(self.prog, [(self.vbo, '4f 2f', 'sPos', 'sTexCoords')], self.ibo)
        ...
        self.depthTexture.use(2)
        
        self.diffTexLoc.value = 0
        self.specTexLoc.value = 1
        self.dispTexLoc.value = 2
        
        self.lightDir_ = np.array([1.0, 1.0, 1.0])
