 ...
 * end, return end.
 *
 * origin: the eye point, acting as the origin of the ray
...
float raymarch(vec3 origin, vec3 direction) {
  ...
  for (int i = 0; i < MAX_ITERATIONS; i++) {
    float dist = f(origin + depth * direction);
    if (dist < EPSILON) {
      return depth;
    }
    depth += dist * 0.5;
...
/**
 * Constructive solid geometry intersection operation on SDF-calculated
 * distances.
 */
...
/**
 * Signed distance function for an XY aligned cylinder centered at the origin
...
float cylinderSDF(vec3 p, float h, float r) {
  ...
  // How far inside or outside the cylinder is, axially aligned with the
  // cylinder
...
vec3 estimateNormal(vec3 p) {
  return normalize(vec3(
      f(vec3(p.x + EPSILON, p.y, p.z)) - f(vec3(p.x - EPSILON, p.y, p.z)),
      f(vec3(p.x, p.y + EPSILON, p.z)) - f(vec3(p.x, p.y - EPSILON, p.z)),
      f(vec3(p.x, p.y, p.z + EPSILON)) - f(vec3(p.x, p.y, p.z - EPSILON))));
}
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float alpha, vec3 p,
  ...
  color +=
      phongContribForLight(k_d, k_s, alpha, p, eye, light1Pos, light1Intensity);

  vec3 light2Pos = vec3(2.0 * sin(0.37 * iTime), 2.0 * cos(0.37 * iTime), 2.0);
  vec3 light2Intensity = vec3(0.4, 0.4, 0.4);

  color +=
      phongContribForLight(k_d, k_s, alpha, p, eye, light2Pos, light2Intensity);
  return color;
}
